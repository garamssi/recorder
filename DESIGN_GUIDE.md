...
- **저장 완료**: 링이 꽉 차고(100%) 역회전 원호가 멈추며 중앙이 체크로 바뀐다.
  ...
  - 판정은 **발행이 확정된 녹화본**(`HomeViewModel.justSaved` ← `completedRecordings`)으로만 한다.
    세션 상태의 `Stopping -> Idle` 전이로 판정하면 발행 실패와 빈 세션도 같은 전이를 만들어
    저장되지 않은 녹화를 "저장했습니다" 로 알린다.
  - 중앙은 체크만 그린다. 길이는 저장 중에 이미 보여 줬고, 완료 순간에 필요한 정보는
    "무엇이 저장됐는가"(파일명)뿐이다. 파일명은 발행 결과에서 읽는다 — 스톱워치가 아니라
    실제 파일이 진실이다.
  - 이 구간에는 녹화 시작 버튼이 없다. 900ms 뒤 스스로 접히고, 버블·알림에서 새 세션이
