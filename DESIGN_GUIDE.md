...
### 저장 완료 배너 (`service/SaveCompleteOverlayWindow.kt`)
발행이 확정된 순간 화면 위에 뜨는 알약 (기능명세서 6.1절 [결정]).

- 화면 **상단 중앙**, 상태 바 아래로 24dp 내려 띄운다. 하단은 제스처 바·버블과 부딪힌다
- 카드색 배경 + 28dp 라운드(알약) + 체크 아이콘(primary) + "녹화를 저장했습니다" 한 줄,
  그 아래 파일명(muted, 말줄임). 무엇이 저장됐는지가 이 표시의 내용이다
- **터치를 가로채지 않는다** (`FLAG_NOT_TOUCHABLE`): 배너 아래의 앱이 그대로 눌린다.
  누를 것이 없으므로 터치를 받을 이유도 없다
- 3초 뒤 스스로 사라진다. 서비스가 먼저 접혀도 남아 있어야 하므로 창의 수명은
  코루틴 스코프가 아니라 메인 스레드 핸들러가 쥔다
- 오버레이 권한이 없으면 토스트로 대체한다 — 그때는 화면에 그릴 수단이 그것뿐이다
...
- **저장 완료**: 링이 꽉 차고(100%) 역회전 원호가 멈추며 중앙이 체크로 바뀐다.
  `SAVED_DISPLAY_MILLIS`(2000ms) 만 머문 뒤 **누를 것 없이 스스로 접혀** 대기로 돌아간다.
  - 이 시간은 **홈이 화면에 보이기 시작한 뒤부터** 센다 (기능명세서 2.1절 [결정]). 백그라운드에서
    흘려보내면 버블로 녹화한 사용자는 돌아왔을 때 이미 접힌 카드를 본다. 눈이 닿는 시점부터
    세므로 900ms 로는 짧다 — 체크로 바뀌는 전환을 알아볼 여유를 준다.
  - 판정은 **발행이 확정된 녹화본**(`HomeViewModel.justSaved` ← `pendingCompletedRecording`)으로만
