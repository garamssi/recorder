...
- 카드색(`BUBBLE_SURFACE`) 배경 + 24dp 라운드. 어떤 앱 위에 얹혀도 읽히려면 면이 필요하다
- 링은 홈과 **같은 값**을 쓴다 — 160dp, 트랙 2dp(recRed 30%), 진행 원호 4dp(recRed, 둥근 끝,
  12시 시계 방향), 역회전 흐린 원호(recRed 25%, 3초/바퀴), recRed 후광 alpha .10↔.28 맥동
  (반지름 1.35배, 중심 비움). 층 구성과 근거는 아래 "홈 > 저장 중" 항목과 같다
- 중앙: 경과 시간 40sp(tabular) + 퍼센트. 발행이 확정되면 링이 꽉 차고 중앙이 체크 44dp로 바뀐다
- 상태 줄: REC 점 + "저장 중…" / "녹화를 저장했습니다", 그 아래 파일명 한 줄(말줄임)
