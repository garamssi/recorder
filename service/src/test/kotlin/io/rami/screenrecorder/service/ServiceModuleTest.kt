...
/**
 * 저장 오버레이의 스코프 (기능명세서 6.1절 [결정]).
 *
 * 오버레이를 띄운 녹화 서비스는 발행 직후 접히고 다음 세션은 새 인스턴스에서 시작된다. 표시가
 * 서비스마다 새로 만들어지면 다음 세션의 `dismiss()` 가 창을 붙인 적 없는 객체를 향해,
 * 지난 표시가 다음 녹화의 첫 프레임에 찍힌다.
 *
...
class ServiceModuleTest {
    @Test
    fun `저장 오버레이는 프로세스에 하나만 둔다`() {
        val provider =
            ServiceModule::class.java.methods.single { it.name == "provideSaveOverlay" }

        assertNotNull(
            "provideSaveOverlay 에 @Singleton 이 없다 — 서비스마다 오버레이가 새로 만들어진다",
