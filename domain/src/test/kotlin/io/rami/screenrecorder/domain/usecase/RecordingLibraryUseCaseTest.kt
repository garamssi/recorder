...
class RecordingLibraryUseCaseTest {
    ...
    private fun recording(name: String, createdAt: Long) = Recording(
    ...
    @Test
    fun `검색어와 정렬 기준을 적용해 목록을 내보낸다`() = runTest {
    ...
    @Test
    fun `유효하지 않은 이름은 저장소 호출 없이 거부한다`() = runTest {
        val useCase = RenameRecordingUseCase(libraryRepository)
