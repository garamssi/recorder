...
class RecordingSessionUseCaseTest {
    ...
    @Test
    fun `유휴 상태이고 공간이 충분하면 녹화를 시작한다`() = runTest {
    ...
    @Test
    fun `저장 공간이 500MB 미만이면 시작을 거부한다`() = runTest {
        givenState(RecordingState.Idle)
    ...
    @Test
    fun `이미 녹화 중이면 시작을 거부한다`() = runTest {
    ...
    @Test
    fun `녹화 중이 아니면 일시정지를 거부한다`() = runTest {
        givenState(RecordingState.Idle)
        val result = PauseRecordingUseCase(sessionRepository)()
    ...
    @Test
    fun `일시정지 상태에서만 재개할 수 있다`() = runTest {
    ...
    @Test
    fun `일시정지 상태가 아니면 재개를 거부한다`() = runTest {
        givenState(RecordingState.Recording(elapsed = 3.seconds))
        val result = ResumeRecordingUseCase(sessionRepository)()
    ...
    @Test
    fun `녹화 중이거나 일시정지 상태면 중지할 수 있다`() = runTest {
        givenState(RecordingState.Recording(elapsed = 3.seconds))
    ...
    @Test
    fun `카운트다운 중에도 중지할 수 있다`() = runTest {
    ...
    @Test
    fun `세션 시작 실패는 Result 실패로 전달된다`() = runTest {
        givenState(RecordingState.Idle)
        givenAvailableBytes(1_000_000_000)
