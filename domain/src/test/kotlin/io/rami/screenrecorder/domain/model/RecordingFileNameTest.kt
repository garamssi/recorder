...
class RecordingFileNameTest {
    ...
    fun `기본 파일명은 접두어_날짜_시각 형식이다`() {
        val name = RecordingFileNameFactory.create(
    ...
    fun `접두어는 영문 숫자 언더스코어만 허용한다`() {
        org.junit.jupiter.api.assertThrows<IllegalArgumentException> { FileNamePrefix("녹화") }
        org.junit.jupiter.api.assertThrows<IllegalArgumentException> { FileNamePrefix("Rec ") }
        org.junit.jupiter.api.assertThrows<IllegalArgumentException> { FileNamePrefix("") }
        org.junit.jupiter.api.assertDoesNotThrow { FileNamePrefix("My_Rec2") }
    }
}
