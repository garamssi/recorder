...
| 부수 1 — 고아 IS_PENDING | 닫힘 | `a083405`, `92e8eba`, `6038358`, `4789529`. id 판정이 DI 스코프 때문에 죽어 있던 것은 `ed693a7` 에서 되살렸다 |
| 부수 2 — 실패 시 임시 파일 삭제 | 닫힘 | `c28cef3` / `596a90d`, 판독 실패 경로는 `b370a47` |
| 부수 3 — readMetadata 부정확 | 닫힘 | 코덱·fps 는 트랙 포맷에서, 크기는 발행된 파일에서 읽는다. **fps 되짚기가 fMP4 에서 값을 주는지는 미검증** |
...
| 알림 클래스가 ID 4종을 혼자 다룸 | 닫힘 | `2e33837` — 수명·ID·취소 정책 축으로 3분할 |
