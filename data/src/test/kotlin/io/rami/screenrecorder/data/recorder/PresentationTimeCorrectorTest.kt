...
class PresentationTimeCorrectorTest {
    private val corrector = PresentationTimeCorrector()
    ...
    fun `일시정지 중 캡처된 샘플은 버린다`() {
        ...
        corrector.onPause(atUs = 150_000L)
    ...
    fun `재개 후 타임스탬프는 일시정지 누적분만큼 당겨진다`() {
        corrector.correct(rawPtsUs = 100_000L)
        corrector.onPause(atUs = 150_000L)
        corrector.onResume(atUs = 1_150_000L) // 1초 일시정지
    ...
    fun `여러 번의 일시정지가 누적 보정된다`() {
        corrector.onPause(atUs = 100_000L)
        corrector.onResume(atUs = 200_000L) // +100ms
        corrector.onPause(atUs = 300_000L)
        corrector.onResume(atUs = 600_000L) // +300ms, 누적 400ms
    ...
    fun `비디오와 오디오에 동일한 보정량이 적용된다`() {
        // 하나의 corrector가 일시정지 오프셋을 소유하고, 트랙별 단조성은 별도 인스턴스로 관리한다.
